...
import {ESTADOS, FIRESTORE_USUARIOS_PATH} from '@/constants';
...
const useSetEstadoUser = () => {
  ...
  const setOnlineUser = useCallback(async (id: string): Promise<void> => {
    try {
      await setDoc(doc(db, FIRESTORE_USUARIOS_PATH, id), {estado: ESTADOS.online}, {merge: true});
    } catch (error: any) {
  ...
  const setOfflineUser = useCallback(async (id: string): Promise<void> => {
    ...
    try {
      await setDoc(doc(db, FIRESTORE_USUARIOS_PATH, id), {estado: ESTADOS.offline}, {merge: true});
    } catch (error: any) {
      toast.error('¡Ups ha ocurrido un error, intenta de nuevo más tarde!');
      DebugUtil.logError(error.message, error);
    } finally {
  ...
  const setInactiveUser = useCallback(async (id: string): Promise<void> => {
    ...
    try {
      await setDoc(doc(db, FIRESTORE_USUARIOS_PATH, id), {estado: ESTADOS.inactivo}, {merge: true});
    } catch (error: any) {
      toast.error('¡Ups ha ocurrido un error, intenta de nuevo más tarde!');
      DebugUtil.logError(error.message, error);
    } finally {
