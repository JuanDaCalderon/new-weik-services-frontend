...
const DateInputColumn = memo(function DateInputColumn({row, registerType, field}: Props) {
  const originalValue = row.original[field] as string;
  const id = row.original.id;
  const [inputValue, setInputValue] = useState<string>(du.formatDateToInput(du.parseStringToDate(originalValue)));
  ...
  const onDispatchAction = useCallback(
    async (e: MouseEvent<FormControlElement> | FocusEvent<FormControlElement>) => {
      ...
      const formattedOriginal = du.formatDateToInput(originalDate)?.trim().toLowerCase();
      const newFormatted = value.trim().toLowerCase();
      if (newFormatted === formattedOriginal || !cliente) return;
      const parsedDate = du.parseDate(value) || new Date();
